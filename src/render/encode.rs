...
const DATA_HEIGHT: u32 = 32; // 4 bytes x 8 bits per column
...
///
/// The image is also stretched 2x horizontally (each column duplicated).
pub fn encode(image: &GrayImage) -> Vec<u8> {
    let (w, h) = image.dimensions();
    // Each output column (after 2x stretch): 4 bytes
    ...
    for x in 0..w {
        ...
        // Stretch 2x: write each column twice
...
/// width here is after 2x stretch.
